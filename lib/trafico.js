class Trafico {
  ...
  async _getMinimumRequiredApprovals(pullRequestLabels) {
    // const { owner, repo } = this.config;

    console.log(pullRequestLabels);

    if (pullRequestLabels) {
      const prSize = pullRequestLabels.find(label => {
        return label.name.startWith("size/");
      }).split("/")[1];

      let requiredApprovals
      switch (prSize) {
        case "XS":
          requiredApprovals = this.config.approvals.XS;
          break;
        case "S":
          requiredApprovals = this.config.approvals.S;
          break;
        case "M":
          requiredApprovals = this.config.approvals.M;
          break;
        case "L":
          requiredApprovals = this.config.approvals.L;
          break;
        case "XL":
          requiredApprovals = this.config.approvals.XL;
          break;
        default:
          break;
      }
